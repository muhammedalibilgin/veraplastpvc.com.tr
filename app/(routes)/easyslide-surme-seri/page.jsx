    ...
    <section className="bg-slate-50 min-h-screen pb-20">
      <div className="relative h-64 mb-10 lg:mb-20 bg-cover bg-center bg-no-repeat flex items-center justify-center" style={{ backgroundImage: `url(/product-lists/page-bg.jpg)` }}>
        ...
        <div className="relative z-10 text-center">
          <h1 className="text-3xl lg:text-5xl font-bold text-white uppercase tracking-wider mb-4">EASY SLIDE SÜRME SERİ</h1>
          <ul className="flex items-center justify-center space-x-2 text-sm">
            ...
            <li className="text-gray-200"><a href="/easyslide-surme-seri" className="text-gray-200 hover:text-white transition-colors" title="Kapılar">EASY SLIDE SÜRME SERİ</a></li>
          </ul>
        </div>
      </div>
      <Container>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-lg:px-4">
          {easyslide.map((item) => (
            <ProductCard key={item.id} product={item} category="easyslide-surme-seri" />
          ))}
        </div>
      </Container>
    </section>
